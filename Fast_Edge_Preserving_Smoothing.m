...
for t=1:T %iteration for separable optimization
 ...
 %%%%Definition of matrix coefficients
        bc_tmp = lambda(t)/(GAMMA*abs(VFx(h,1))^UPSILON+1);
        b(1) = 1+bc_tmp;
        c(1) = 0-bc_tmp;
        d(1) = Zinit(h, 1);
        for i = 2:W-1
            ab_tmp = lambda(t)/(GAMMA*abs(VFx(h,i-1))^UPSILON+1);
            bc_tmp = lambda(t)/(GAMMA*abs(VFx(h,i))^UPSILON+1);
            a(i-1) = 0-ab_tmp;
            b(i) = 1+ab_tmp+bc_tmp;
            c(i) = 0-bc_tmp;
            d(i) = Zinit(h, i);            
        end
        ab_tmp = lambda(t)/(GAMMA*abs(VFx(h,W-1))^UPSILON+1); 
        b(W) = 1+ab_tmp;
        a(W-1) = 0-ab_tmp;
        d(W) = Zinit(h, W);
  ...
  %%%%Definition of matrix coefficients       
        bc_tmp = lambda(t)/(GAMMA*abs(VFy(1,w))^UPSILON+1);
        bb(1) = 1+bc_tmp;
        cc(1) = 0-bc_tmp;
        dd(1) = Zinit(1,w);
        for i = 2:H-1
            ab_tmp = lambda(t)/(GAMMA*abs(VFy(i-1,w))^UPSILON+1);
            bc_tmp = lambda(t)/(GAMMA*abs(VFy(i,w))^UPSILON+1);
            aa(i-1) = 0-ab_tmp;
            bb(i) = 1+ab_tmp+bc_tmp;
            cc(i) = 0-bc_tmp;
            dd(i) = Zinit(i,w);            
        end
        ab_tmp = lambda(t)/(GAMMA*abs(VFy(H-1,w))^UPSILON+1); 
        bb(H) = 1+ab_tmp;
        aa(H-1) = 0-ab_tmp;
        dd(H) = Zinit(H, w);         
  %%%%1D solver along x direction                        
